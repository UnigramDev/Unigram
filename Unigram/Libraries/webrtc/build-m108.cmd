...
for %%a in (%~1) do (
    for %%c in (%~2) do (
        ...
        call gn gen --ide=vs2022 out\msvc\uwp\%%c\%%a --filters=//:webrtc "--args=is_debug=!is_debug! use_lld=false is_clang=false rtc_include_tests=false rtc_build_tools=false rtc_win_video_capture_winrt=true target_os=\"winuwp\" rtc_build_examples=false rtc_win_use_mf_h264=true enable_libaom=false rtc_enable_protobuf=false rtc_disable_metrics=true rtc_include_dav1d_in_internal_decoder_factory=false treat_warnings_as_errors=false use_custom_libcxx=false libcxx_is_shared=true fatal_linker_warnings=false target_cpu=\"%%a\""
        if errorlevel 1 goto :error
