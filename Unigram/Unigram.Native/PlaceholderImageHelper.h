...
namespace Unigram
{
	namespace Native
	{
		...
		{
		public:
			static void DrawProfilePlaceholder(Color clear, _In_ Platform::String^ text, _In_ IRandomAccessStream^ randomAccessStream);
			static void DrawThumbnailPlaceholder(_In_ Platform::String^ fileName, float blurAmount, _In_ IRandomAccessStream^ randomAccessStream);

		internal:
			PlaceholderImageHelper();

		private:
			static property PlaceholderImageHelper^ Instance
			{
