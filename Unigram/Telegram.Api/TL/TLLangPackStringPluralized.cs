...
namespace Telegram.Api.TL
{
	public partial class TLLangPackStringPluralized : TLLangPackStringBase 
	{
		...
		public override void Write(TLBinaryWriter to)
		{
			...
			to.Write(Key);
			if (HasZeroValue) to.Write(ZeroValue);
			if (HasOneValue) to.Write(OneValue);
			if (HasTwoValue) to.Write(TwoValue);
			if (HasFewValue) to.Write(FewValue);
			if (HasManyValue) to.Write(ManyValue);
			to.Write(OtherValue);
		}
