...
#warning TODO: Replace BootStrapper.Current.Compositor with parameter?
        public static CompositionAnimation CreateThumbnail(float width, float height, CompositionPath path, out ShapeVisual visual, bool animated = true)
        {
            ...
            gradient.ColorStops.Add(BootStrapper.Current.Compositor.CreateColorGradientStop(0.0f, transparent));
            gradient.ColorStops.Add(BootStrapper.Current.Compositor.CreateColorGradientStop(0.5f, foregroundColor));
            gradient.ColorStops.Add(BootStrapper.Current.Compositor.CreateColorGradientStop(1.0f, transparent));

            var background = BootStrapper.Current.Compositor.CreateRectangleGeometry();
            background.Size = new Vector2(width, height);
            var backgroundShape = BootStrapper.Current.Compositor.CreateSpriteShape(background);
            backgroundShape.FillBrush = BootStrapper.Current.Compositor.CreateColorBrush(backgroundColor);

            var foreground = BootStrapper.Current.Compositor.CreateRectangleGeometry();
            foreground.Size = new Vector2(width, height);
            var foregroundShape = BootStrapper.Current.Compositor.CreateSpriteShape(foreground);
            foregroundShape.FillBrush = gradient;

            var clip = BootStrapper.Current.Compositor.CreateGeometricClip(BootStrapper.Current.Compositor.CreatePathGeometry(path));
            clip.ViewBox = BootStrapper.Current.Compositor.CreateViewBox();
            clip.ViewBox.Size = new Vector2(width, height);
            clip.ViewBox.Stretch = CompositionStretch.UniformToFill;

            visual = BootStrapper.Current.Compositor.CreateShapeVisual();
            visual.Clip = clip;
            visual.Shapes.Add(backgroundShape);
            visual.Shapes.Add(foregroundShape);
            visual.RelativeSizeAdjustment = Vector2.One;
            visual.ViewBox = BootStrapper.Current.Compositor.CreateViewBox();
