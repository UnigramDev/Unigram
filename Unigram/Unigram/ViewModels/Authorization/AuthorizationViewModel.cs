    ...
    public class AuthorizationViewModel : TLViewModelBase, IDelegable<ISignInDelegate>
    {
        ...
        private async void SendExecute()
        {
            ...
            foreach (var session in _lifetimeService.Items)
            {
                ...
                {
                    var confirm = await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.AccountAlreadyLoggedIn, Strings.Resources.AppName, Strings.Resources.AccountSwitch, Strings.Resources.OK);
            ...
            if (response is Error error)
            {
                ...
                if (error.TypeEquals(ErrorType.PHONE_NUMBER_INVALID))
                {
                    ...
                    await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.InvalidPhoneNumber, Strings.Resources.AppName, Strings.Resources.OK);
                }
                else if (error.TypeEquals(ErrorType.PHONE_PASSWORD_FLOOD))
                {
                    await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.FloodWait, Strings.Resources.AppName, Strings.Resources.OK);
                }
                else if (error.TypeEquals(ErrorType.PHONE_NUMBER_FLOOD))
                {
                    await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.PhoneNumberFlood, Strings.Resources.AppName, Strings.Resources.OK);
                }
                else if (error.TypeEquals(ErrorType.PHONE_NUMBER_BANNED))
                {
                    //needShowInvalidAlert(req.phone_number, true);
                    await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.BannedPhoneNumber, Strings.Resources.AppName, Strings.Resources.OK);
                }
                else if (error.TypeEquals(ErrorType.PHONE_CODE_EMPTY) || error.TypeEquals(ErrorType.PHONE_CODE_INVALID))
                {
                    await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.InvalidCode, Strings.Resources.AppName, Strings.Resources.OK);
                }
                else if (error.TypeEquals(ErrorType.PHONE_CODE_EXPIRED))
                {
                    await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.CodeExpired, Strings.Resources.AppName, Strings.Resources.OK);
                }
