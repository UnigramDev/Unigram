    ...
    public partial class DialogViewModel
    {
        ...
        private async void MessageReportFalsePositiveExecute(MessageViewModel message)
        {
            if (_chat?.Type is ChatTypeSupergroup supergroup)
            {
                ...
                await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.ChannelAntiSpamFalsePositiveReported);
            }
        }
        ...
        public async void KeyboardButtonInline(MessageViewModel message, InlineKeyboardButton inline)
        {
            ...
            {
                await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.MessageScheduledBotAction, Strings.Resources.AppName, Strings.Resources.OK);
                return;
            }
            ...
            else if (inline.Type is InlineKeyboardButtonTypeUrl urlButton)
            {
                if (MessageHelper.TryCreateUri(urlButton.Url, out Uri uri))
                {
                    ...
                    {
                        var confirm = await MessagePopup.ShowAsync(XamlRoot, string.Format(Strings.Resources.OpenUrlAlert, urlButton.Url), Strings.Resources.AppName, Strings.Resources.OK, Strings.Resources.Cancel);
                        if (confirm != ContentDialogResult.Primary)
                        {
                            return;
                        }
            ...
            else if (inline.Type is InlineKeyboardButtonTypeCallback callback)
            {
                ...
                if (response is CallbackQueryAnswer answer)
                {
                    ...
                    if (!string.IsNullOrEmpty(answer.Text))
                    {
                        if (answer.ShowAlert)
                        {
                            await new MessagePopup(answer.Text).ShowQueuedAsync(XamlRoot);
                        }
                        else
                        {
                            if (bot == null)
            ...
            else if (inline.Type is InlineKeyboardButtonTypeCallbackWithPassword callbackWithPassword)
            {
                ...
                if (response is Error error)
                {
                    if (error.Message.Equals("PASSWORD_MISSING") || error.Message.StartsWith("PASSWORD_TOO_FRESH_") || error.Message.StartsWith("SESSION_TOO_FRESH_"))
                    {
                        ...
                        var confirm = await MessagePopup.ShowAsync(XamlRoot, builder.ToString(), Strings.Resources.EditAdminTransferAlertTitle, primary, Strings.Resources.Cancel);
            ...
            else if (inline.Type is InlineKeyboardButtonTypeWebApp webApp)
            {
                ...
                {
                    await new WebBotPopup(SessionId, webAppInfo, inline.Text).ShowQueuedAsync(XamlRoot);
                }
            }
        }
        ...
        public async void KeyboardButtonExecute(MessageViewModel message, KeyboardButton keyboardButton)
        {
            ...
            if (keyboardButton.Type is KeyboardButtonTypeRequestPhoneNumber)
            {
                if (ClientService.TryGetUser(ClientService.Options.MyId, out Telegram.Td.Api.User cached))
                {
                    ...
                    var confirm = await MessagePopup.ShowAsync(XamlRoot, content, Strings.Resources.ShareYouPhoneNumberTitle, Strings.Resources.OK, Strings.Resources.Cancel);
                    if (confirm == ContentDialogResult.Primary)
                    {
            ...
            {
                var confirm = await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.ShareYouLocationInfo, Strings.Resources.ShareYouLocationTitle, Strings.Resources.OK, Strings.Resources.Cancel);
                if (confirm == ContentDialogResult.Primary)
                {
            ...
            else if (keyboardButton.Type is KeyboardButtonTypeWebApp webApp && message.SenderId is MessageSenderUser bot)
            {
                ...
                if (response is WebAppInfo webAppInfo)
                {
                    await new WebBotPopup(SessionId, webAppInfo, keyboardButton.Text).ShowQueuedAsync(XamlRoot);
                }
            }
        }
        ...
        public async void OpenWebView()
        {
            ...
            if (ContentDialogResult.Primary != await MessagePopup.ShowAsync(XamlRoot, string.Format(Strings.Resources.BotOpenPageMessage, chat.Title), Strings.Resources.BotOpenPageTitle, Strings.Resources.OK, Strings.Resources.Cancel))
            {
                return;
            }
        }
        ...
        public async void MessageServiceExecute(MessageViewModel message)
        {
            ...
            else if (message.Content is MessageChatEvent chatEvent)
            {
                ...
                {
                    await StickersPopup.ShowAsync(XamlRoot, stickerSetChanged.NewStickerSetId);
                }
            }
            else if (message.Content is MessageVideoChatStarted or MessageVideoChatScheduled)
            {
                await _groupCallService.JoinAsync(XamlRoot, message.ChatId);
            }
        ...
        private async void MessageStopPollExecute(MessageViewModel message)
        {
            ...
            var confirm = await MessagePopup.ShowAsync(XamlRoot, Strings.Resources.StopPollAlertText, Strings.Resources.StopPollAlertTitle, Strings.Resources.Stop, Strings.Resources.Cancel);
            if (confirm != ContentDialogResult.Primary)
            {
                return;
            }
        ...
        private async void MessageShowEmojiExecute(MessageViewModel message)
        {
            ...
            if (response is Stickers stickers)
            {
                ...
                await StickersPopup.ShowAsync(XamlRoot, sets);
            }
        }
