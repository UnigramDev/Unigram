            ...
            ProtoService.Send(new GetInstalledStickerSets(_masks), result =>
            {
                if (result is StickerSets stickerSets)
                {
                    ProtoService.Send(new GetRecentStickers(_masks), resultRecent =>
                    {
                        ...
                        {
                            BeginOnUIThread(() => Items.ReplaceWith(new[] { new StickerSetInfo(0, Strings.Resources.RecentStickers, "tg/recentlyUsed", null, false, false, false, _masks, false, recents.StickersValue.Count, recents.StickersValue) }.Union(stickerSets.Sets)));
                        }
                        else
                        {
