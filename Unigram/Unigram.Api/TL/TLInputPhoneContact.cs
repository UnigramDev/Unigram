// <auto-generated/>
using System;

namespace Telegram.Api.TL
{
	public partial class TLInputPhoneContact : TLInputContactBase 
	{
		public Int64 ClientId { get; set; }
		public String Phone { get; set; }
		public String FirstName { get; set; }
		public String LastName { get; set; }

		public TLInputPhoneContact() { }
		public TLInputPhoneContact(TLBinaryReader from, TLType type = TLType.InputPhoneContact)
		{
			Read(from, type);
		}

		public override TLType TypeId { get { return TLType.InputPhoneContact; } }

		public override void Read(TLBinaryReader from, TLType type = TLType.InputPhoneContact)
		{
			ClientId = from.ReadInt64();
			Phone = from.ReadString();
			FirstName = from.ReadString();
			LastName = from.ReadString();
		}

		public override void Write(TLBinaryWriter to)
		{
			to.Write(0xF392B7F4);
			to.Write(ClientId);
			to.Write(Phone);
			to.Write(FirstName);
			to.Write(LastName);
		}
	}
}