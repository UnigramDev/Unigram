...
namespace Telegram.Api.TL
{
	public partial class TLInputBotInlineResult : TLInputBotInlineResultBase 
	{
		...
		public TLInputBotInlineResult(TLBinaryReader from)
		{
			Read(from);
		}
		...
		public override void Write(TLBinaryWriter to)
		{
			...
			if (HasThumbUrl) to.WriteString(ThumbUrl ?? string.Empty);
			if (HasContentUrl) to.WriteString(ContentUrl ?? string.Empty);
			if (HasContentType) to.WriteString(ContentType ?? string.Empty);
			if (HasW) to.WriteInt32(W.Value);
			if (HasH) to.WriteInt32(H.Value);
