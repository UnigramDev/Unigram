// <auto-generated/>
using System;

namespace Telegram.Api.TL
{
	public partial class TLMessageMediaContact : TLMessageMediaBase 
	{
		public String PhoneNumber { get; set; }
		public String FirstName { get; set; }
		public String LastName { get; set; }
		public Int32 UserId { get; set; }

		public TLMessageMediaContact() { }
		public TLMessageMediaContact(TLBinaryReader from, TLType type = TLType.MessageMediaContact)
		{
			Read(from, type);
		}

		public override TLType TypeId { get { return TLType.MessageMediaContact; } }

		public override void Read(TLBinaryReader from, TLType type = TLType.MessageMediaContact)
		{
			PhoneNumber = from.ReadString();
			FirstName = from.ReadString();
			LastName = from.ReadString();
			UserId = from.ReadInt32();
		}

		public override void Write(TLBinaryWriter to)
		{
			to.Write(0x5E7D2F39);
			to.Write(PhoneNumber);
			to.Write(FirstName);
			to.Write(LastName);
			to.Write(UserId);
		}
	}
}