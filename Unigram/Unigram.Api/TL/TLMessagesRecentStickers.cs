// <auto-generated/>
using System;

namespace Telegram.Api.TL
{
	public partial class TLMessagesRecentStickers : TLMessagesRecentStickersBase 
	{
		public Int32 Hash { get; set; }
		public TLVector<TLDocumentBase> Stickers { get; set; }

		public TLMessagesRecentStickers() { }
		public TLMessagesRecentStickers(TLBinaryReader from, TLType type = TLType.MessagesRecentStickers)
		{
			Read(from, type);
		}

		public override TLType TypeId { get { return TLType.MessagesRecentStickers; } }

		public override void Read(TLBinaryReader from, TLType type = TLType.MessagesRecentStickers)
		{
			Hash = from.ReadInt32();
			Stickers = TLFactory.Read<TLVector<TLDocumentBase>>(from);
		}

		public override void Write(TLBinaryWriter to)
		{
			to.Write(0x5CE20970);
			to.Write(Hash);
			to.WriteObject(Stickers);
		}
	}
}