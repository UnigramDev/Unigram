// <auto-generated/>
using System;

namespace Telegram.Api.TL
{
	public partial class TLMessageEntityMentionName : TLMessageEntityBase 
	{
		public Int32 UserId { get; set; }

		public TLMessageEntityMentionName() { }
		public TLMessageEntityMentionName(TLBinaryReader from, TLType type = TLType.MessageEntityMentionName)
		{
			Read(from, type);
		}

		public override TLType TypeId { get { return TLType.MessageEntityMentionName; } }

		public override void Read(TLBinaryReader from, TLType type = TLType.MessageEntityMentionName)
		{
			Offset = from.ReadInt32();
			Length = from.ReadInt32();
			UserId = from.ReadInt32();
		}

		public override void Write(TLBinaryWriter to)
		{
			to.Write(0x352DCA58);
			to.Write(Offset);
			to.Write(Length);
			to.Write(UserId);
		}
	}
}