...
namespace Telegram.Generators
{
    ...
    public class SchemaGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            var content = files.Select((file, _) => file.GetText()?.ToString());

            context.RegisterSourceOutput(content, (ctx, text) =>
            {
                if (text is null) return;

                ctx.AddSource("TdDotNetApi.g.cs", Main(text));
            });
        }
        ...
        static string Main(string text)
        {
            try
            {
                var classes = Parse(text.Replace("\r\n", "\n"))
                    .OrderBy(x => x.IsFunction)
                    .ThenBy(x => x.Name)
                    .ToList();

                var functionTypes = classes.Where(x => x.IsFunction).Select(x => x.ReturnType).Distinct().ToList();
                var abstractTypes = classes
                    .Where(x => !x.IsFunction && x.ReturnType != null)
                    .GroupBy(x => to_CamelCase(x.ReturnType ?? x.Name))
                    .ToDictionary(x => x.Key, y => y.ToList());

                var returnTypes = classes
                    .Where(x => !x.IsFunction)
                    .GroupBy(x => to_CamelCase(x.ReturnType ?? x.Name))
                    .Where(x => functionTypes.Any(y => y == x.Key))
                    .SelectMany(x => x)
                    .Where(x => !x.IsProxy)
                    .ToList();
                ...
                {
                    WriteClassDefinition(builder, type, toJsonTypes.Contains(type));
                }
                ...
                foreach (var type in classes)
                {
                    if (type.IsFunction || fromJsonTypes.Contains(type))
                    {
                        continue;
                    }
                    ...
                    if (abstractTypes.TryGetValue(type.Name, out var innerTypes))
                    {
                        WriteFromJson(builder, type.Name, innerTypes);
                    }
                    else
                    {
                        WriteToJson(builder, type);
                    }
                }
        ...
        private static List<SchemaClass> FindReferencedTypes(List<SchemaClass> classes, IEnumerable<SchemaClass> functionTypes)
        {
            var abstractTypes = classes
                .Where(x => !x.IsFunction)
                .GroupBy(x => to_CamelCase(x.ReturnType ?? x.Name))
            ...
            var items = new List<SchemaClass>();

            void Process(SchemaClass type)
            {
                ...
                {
                    var baseType = to_CamelCase(prop.Type);
                    if (abstractTypes.TryGetValue(baseType, out var innerTypes))
                    {
                        innerTypes.ForEach(Process);
                    }
                }
            }
        ...
        static string to_CamelCase(string name)
        {
        ...
        static string to_cCamelCase(string name, bool flag)
        {
        ...
        private static string GetPropertyType(SchemaProperty property)
        {
            static string Case(string name)
            {
                if (name == "Bool")
                {
                    return "bool";
        ...
        private static void WriteFromJson(StringBuilder builder, string name, List<SchemaClass> classes)
        {
            var className = to_CamelCase(name);
        ...
        private static void WriteFromJson(StringBuilder builder, SchemaClass type)
        {
            ...
            var crc32 = ComputeCrc32(Encoding.UTF8.GetBytes(type.Name));
            builder.AppendLine("        case " + crc32 + ":");
            builder.AppendLine("            return FromJson_" + to_CamelCase(type.Name) + "(ref reader, handler);");
        }

        private static void WriteToJson(StringBuilder builder, SchemaClass type)
        {
            var className = to_CamelCase(type.Name);
            builder.AppendLine("private static " + className + " FromJson_" + className + "(ref Utf8JsonReader reader, ClientResultHandler handler)");
            builder.AppendLine("{");

            if (className == "UpdateFile")
            {
                builder.AppendLine("    return handler.ParseUpdateFile(ref reader);");
            }
            else if (className == "UpdateFile" || className == "File")
            {
                builder.AppendLine("    return handler.ParseFile(ref reader);");
            }
            ...
            else if (type.Properties.Count > 0)
            {
                ...
                foreach (var prop in type.Properties)
                {
                    var crc32 = ComputeCrc32(Encoding.UTF8.GetBytes(prop.Name));
                    var fieldName = to_CamelCase(prop.Name);
                    ...
                    if (prop.IsVector)
                    {
                        if (prop.Type == "Bool")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetBooleanArray();");
                        }
                        else if (prop.Type == "int32")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetInt32Array();");
                        }
                        else if (prop.Type == "int53")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetInt64Array();");
                        }
                        else if (prop.Type == "int64")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetInt64StringArray();");
                        }
                        else if (prop.Type == "double")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetDoubleArray();");
                        }
                        else if (prop.Type == "string")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetStringArray();");
                        }
                        else if (prop.Type == "bytes")
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetBase64StringArray();");
                        }
                        else if (prop.IsVectorOfVectors)
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetObjectArrayArray(handler, FromJson_" + to_CamelCase(prop.Type) + ");");
                        }
                        else
                        {
                            builder.AppendLine("                obj." + fieldName + " = reader.GetObjectArray(handler, FromJson_" + to_CamelCase(prop.Type) + ");");
                        }
                    }
                    else if (prop.Type == "Bool")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetBoolean();");
                    }
                    else if (prop.Type == "int32")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetInt32();");
                    }
                    else if (prop.Type == "int53")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetInt64();");
                    }
                    else if (prop.Type == "int64")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetInt64String();");
                    }
                    else if (prop.Type == "double")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetDouble();");
                    }
                    else if (prop.Type == "string")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetString();");
                    }
                    else if (prop.Type == "bytes")
                    {
                        builder.AppendLine("                obj." + fieldName + " = reader.GetBytesFromBase64();");
                    }
                    else
                    {
                        builder.AppendLine("                obj." + fieldName + " = FromJson_" + to_CamelCase(prop.Type) + "(ref reader, handler);");
                    }
        ...
        private static void WriteClassDefinition(StringBuilder builder, SchemaClass type, bool serializable)
        {
            var className = to_CamelCase(type.Name);
            var baseName = to_CamelCase(type.IsFunction ? "Function" : type.ReturnType ?? "Object");
            ...
            if (type.IsFunction)
            {
                builder.AppendLine("/// <para>Returns <see cref=\"T:Telegram.Td.Api." + to_CamelCase(type.ReturnType) + "\"/>.</para>");
            }
            ...
            else
            {
                ...
                foreach (var prop in type.Properties)
                {
                    ...
                    {
                        builder.AppendLine("    public " + GetPropertyType(prop) + " " + fieldName + " { get; set; } = string.Empty;");
                    }
                    else
                    {
                        builder.AppendLine("    public " + GetPropertyType(prop) + " " + fieldName + " { get; set; }");
                    }
                }
                ...
                if (type.Properties.Count > 0)
                {
                    ...
                    foreach (var prop in type.Properties)
                    {
                        ...
                        builder.AppendLine("    /// <param name=\"" + fieldName + "\">" + prop.Description + "</param>");
                    }
                    builder.Append("    public " + className + "(");

                    for (int i = 0; i < type.Properties.Count; i++)
                    {
                        SchemaProperty property = type.Properties[i];
                        var fieldName = to_camelCase(property.Name);
                        if (fieldName == "event")
                        {
                            fieldName = "@" + fieldName;
                        }
                        ...
                        builder.Append(GetPropertyType(property) + " " + fieldName);
                    }
                    ...
                    foreach (var property in type.Properties)
                    {
                        ...
                        var inlineName = to_camelCase(property.Name);
                        if (inlineName == "event")
                        {
                            inlineName = "@" + inlineName;
                        }

                        builder.AppendLine("        " + fieldName + " = " + inlineName + ";");
                    }
        ...
        private static List<SchemaClass> Parse(string text)
        {
            ...
            while (true)
            {
                ...
                if (reader.TokenType == TokenType.Functions)
                {
                    functions = true;
                    reader.Read();
                }
                else if (reader.TokenType == TokenType.Comment)
                {
                    documentation.Clear();
                ...
                else if (reader.TokenType == TokenType.Object)
                {
                    var split = reader.Value.Split('=');
                    var definition = split[0].TrimEnd();
                    var returnType = split[1].TrimStart().TrimEnd(';');

                    split = definition.Split(' ');

                    var className = split[0];
                    var properties = new List<SchemaProperty>(split.Length - 1);

                    for (int i = 1; i < split.Length; i++)
                    {
                        string prop = split[i];
                        var data = prop.Split(':');
                        var desc = documentation[data[0]];
                        if (desc.Contains("; for bots only") && !includeBotsOnly)
                        {
                            continue;
                        }

                        properties.Add(new SchemaProperty(data[0], data[1], desc));
                    }

                    if (to_CamelCase(className) == to_CamelCase(returnType))
                    {
                        returnType = null;
                    }
        ...
        private static void ParseDocumentationLine(Dictionary<string, string> items, string line)
        {
        ...
        public static uint Crc32Partial(ReadOnlySpan<byte> data, uint crc)
        {
            ...
            // Process 32 bytes (8 x 4-byte ints) at a time
            int blocks32 = len >> 5;
            if (blocks32 > 0)
            {
                ...
                for (int i = 0; i < blocks32; i++)
                {
                    // Process 8 uints (32 bytes)
                    crc = ProcessUInt(crc, uintSpan[uintIndex++]);
                    crc = ProcessUInt(crc, uintSpan[uintIndex++]);
                    crc = ProcessUInt(crc, uintSpan[uintIndex++]);
                    crc = ProcessUInt(crc, uintSpan[uintIndex++]);
                    crc = ProcessUInt(crc, uintSpan[uintIndex++]);
            ...
            // Process remaining 16-byte block
            if ((len & 16) != 0)
            {
                ReadOnlySpan<uint> uintSpan = MemoryMarshal.Cast<byte, uint>(data.Slice(offset));
                crc = ProcessUInt(crc, uintSpan[0]);
                crc = ProcessUInt(crc, uintSpan[1]);
                crc = ProcessUInt(crc, uintSpan[2]);
                crc = ProcessUInt(crc, uintSpan[3]);
                offset += 16;
            }

            // Process remaining 8-byte block
            if ((len & 8) != 0)
            {
                ReadOnlySpan<uint> uintSpan = MemoryMarshal.Cast<byte, uint>(data.Slice(offset));
                crc = ProcessUInt(crc, uintSpan[0]);
            ...
            if ((len & 2) != 0)
            {
                ...
                crc = crc32_table[(crc ^ data[offset + 1]) & 0xFF] ^ (crc >> 8);
                offset += 2;
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static uint ProcessUInt(uint crc, uint value)
        {
            crc ^= value;
            return crc32_table0[crc & 0xFF] ^
                   crc32_table1[(crc >> 8) & 0xFF] ^
                   crc32_table2[(crc >> 16) & 0xFF] ^
                   crc32_table[crc >> 24];
        }
        ...
        private class Reader
        {
            ...
            public Reader(string text)
            {
                ...
                while (!_lines[_index].StartsWith("//"))
                {
                    _index++;
                }
            }
            ...
            public bool Read()
            {
                if (_lines[_index].StartsWith("//"))
                {
                    _tokenType = TokenType.Comment;
                    Value = _lines[_index].Substring(2);
                ...
                else if (_lines[_index] == "---functions---")
                {
                    _tokenType = TokenType.Functions;
                    Value = string.Empty;
                }
                else if (string.IsNullOrEmpty(_lines[_index]))
                {
                    var temp = _index + 1;
                    while (temp < _lines.Length && string.IsNullOrEmpty(_lines[temp]))
                    {
                        temp++;
                    }
        ...
        {
            public SchemaProperty(string name, string type, string documentation)
            {
            ...
            public bool IsVectorOfVectors { get; set; }
