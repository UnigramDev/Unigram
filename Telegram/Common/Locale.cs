...
namespace Telegram.Common
{
    ...
    public static partial class Locale
    {
        ...
        // char is not blittable while runtime marshalling is enabled, which is where a UWP app
        // stays, so the buffer crosses as a pointer rather than as Span<char>.
        [LibraryImport("kernel32.dll")]
        private static unsafe partial int GetUserDefaultLocaleName(char* buf, int bufferLength);
        ...
        public static unsafe string GetUserDefaultLocaleName()
        {
            ...
            {
                result = GetUserDefaultLocaleName(ptr, buffer.Length);
            }
            if (result != 0)
            {
        ...
        {
            var diff = DateTime.Now.ToUnixTimeSeconds() - date;
            if (diff <= 0)
            {
        ...
        public static string FormatLivePeriod(int livePeriod, int date)
        {
            ...
            var diff = date + livePeriod - DateTime.Now.ToUnixTimeSeconds();
            if (diff < 60 * 60)
            {
        ...
        public static string FormatLocationUpdateDate(long date)
        {
            try
            {
                ...
                if (dateDay == day && year == dateYear)
                {
                    ...
                    var diff = (int)(DateTime.Now.ToUnixTimeSeconds() / 1000 - date) / 60;
                    if (diff < 1)
                    {
                ...
                else if (Math.Abs(DateTime.Now.ToUnixTimeSeconds() / 1000 - date) < 31536000000L)
                {
