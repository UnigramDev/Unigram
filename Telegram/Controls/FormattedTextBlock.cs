        ...
        public void SetText(IClientService clientService, StyledText styled, double fontSize = 0)
        {
            ...
            foreach (var part in styled.Paragraphs)
            {
                ...
                if (part.Type is TextParagraphTypeQuote)
                {
                    ...
                    var temp = paragraph as Paragraph;
                ...
                foreach (var entity in runs)
                {
                    ...
                    {
                        paragraph.Inlines.Add(CreateRun(text.Substring(previous, entity.Offset - previous), direction, fontSize: fontSize));
                    }
                    ...
                    if (entity.HasFlag(Common.TextStyle.Monospace))
                    {
                        ...
                        {
                            paragraph.FontFamily = new FontFamily("Consolas, " + Theme.Current.XamlAutoFontFamily);
                    ...
                    else
                    {
                        ...
                        if (entity.Type is TextEntityTypeCustomEmoji customEmoji && ((_ignoreSpoilers && entity.HasFlag(Common.TextStyle.Spoiler)) || !entity.HasFlag(Common.TextStyle.Spoiler)))
                        {
                            ...
                            {
                                paragraph.Inlines.Add(CreateRun(direction == FlowDirection.RightToLeft ? Icons.RTL : Icons.LTR, direction));
                            }

                            // TODO: see if there's a better way
                            paragraph.Inlines.Add(inline);
                            paragraph.Inlines.Add(CreateRun(Icons.ZWNJ, direction));

                            workaround++;
                        }
                        else
                        {
                            var run = CreateRun(text.Substring(entity.Offset, entity.Length), direction, fontSize: fontSize);
                ...
                {
                    paragraph.Inlines.Add(CreateRun(text.Substring(previous), direction, fontSize: fontSize));
                }

                // ZWJ is added to workaround a crash caused by emoji ad the end of a paragraph that is being highlighted
                paragraph.Inlines.Add(CreateRun(Icons.ZWJ, direction));
