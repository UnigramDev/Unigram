...
namespace Telegram.ViewModels
{
    public class DialogBusinessRepliesViewModel : DialogViewModel, IDiffHandler<MessageViewModel>
    {
        ...
        private void Handle(UpdateQuickReplyShortcutMessages update)
        {
            ...
            var replied = update.Messages.OrderBy(x => x.Id).Select(x => CreateMessage(new Message(x.Id, new MessageSenderUser(ClientService.Options.MyId), ClientService.Options.MyId, x.SendingState, null, true, false, x.CanBeEdited, false, false, false, false, true, false, false, false, false, false, false, false, false, false, false, false, 0, 0, null, null, null, null, null, 0, 0, null, 0, 0, x.ViaBotUserId, 0, string.Empty, x.MediaAlbumId, string.Empty, x.Content, x.ReplyMarkup))).ToList();
