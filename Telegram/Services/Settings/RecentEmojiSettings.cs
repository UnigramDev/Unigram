...
{
    // Recent emoji are per account: an entry can be "{emoji};{customEmojiId}", and a custom emoji
    // id only resolves on the account that stored it. Skin tones stay app-wide on EmojiSettings --
    // nothing about a skin tone belongs to an account.
    public partial class RecentEmojiSettings : SettingsServiceBase
    {
        ...
        public RecentEmojiSettings(ISettingsStore container)
            : base(container.GetContainer("Emoji"))
        {
        }
