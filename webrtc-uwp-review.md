...
| 49 | .cc 939-942 | B | `syncBuffer = new BYTE[syncBufferSize];` followed by `if (syncBuffer == nullptr)`. Throwing `new` never returns null, so the check is dead; with exceptions disabled the allocation terminates the process instead. Directly relevant to the memory-pressure crashes. | fixed 24fb5d13 |
...
| 59 | .cc 1274 | B | The **capture** `InitTransport` assigns `_audioFrameSize`, `_sampleRate` and `_blockSize` only inside `if (hr == S_OK)` after the format search, then calls `IAudioClient::Initialize` regardless — which in shared mode can succeed with a convertible format. `_blockSize` then stays 0 and the capture thread computes `(10 * syncBufIndex) / _blockSize`, an integer divide by zero, and loops `while (syncBufIndex >= _blockSize)` subtracting nothing. **The render side is not affected:** it has an `hr == S_FALSE && pWfxOut` branch that adopts the device mix format, sets all four fields and initialises `PushResampler`, which is the designed path for 44.1 kHz endpoints. `c419936d` wrongly treated that as a failure and was reverted in `aedeae99`. The capture remedy is probably a matching fallback rather than a hard error. | open |
